...
const createRequest = async (req, res) => {
  const reqs = req.body;
  const Request = new TransportRequest(reqs); // It is exported in routes so dont need to export it here
...
const deleteRequest = async (req, res) => {
  ...
  try {
    await TransportRequest.findByIdAndDelete(id); //exec to make it a executable function
...
const getOneRequest = async (req, res) => {
  ...
  const req = await TransportRequest.findById(id)
    .then((reqs) => {
