...
const getRequests = async (req, res) => {
  try {
    const allReqs = await TransportManagement.find(); //Will get All information
...
const createRequest = async (req, res) => {
  const reqs = req.body;
  const Request = new TransportManagement(reqs); // It is exported in routes so dont need to export it here
...
const deleteRequest = async (req, res) => {
  ...
  try {
    await TransportManagement.findByIdAndDelete(id); //exec to make it a executable function
...
const updateRequest = async (req, res) => {
  ...
  const body = req.body;
  try {
    await TransportManagement.findByIdAndUpdate(id, body);
...
const getOneRequest = async (req, res) => {
  ...
  const request = await TransportManagement.findById(id)
    .then((reqs) => {
