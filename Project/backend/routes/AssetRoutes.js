...
router.route("/update/:id").put(async (req,res) => {
    let id = req.params.id;  
      const {invenID, availibilityStatus,roomId,category} = req.body;
...
router.route("/delete/:id").delete(async(req, res) => {
    ...
    .then(() => {
        ...
        res.status(500).send({status: "Error with deleting assets", error: err.message});
