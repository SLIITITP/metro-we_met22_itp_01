...
//http://localhost:8080/shift/create
...
//http://localhost:8080/shift/read
...
//http://localhost:8080/shift/read/id
//Read data from the database
...
router.route("/read/:id").get(async (req, res) => {
    ...
    .then((shift) => {
        ...
        res.status(500).send({status: "Error with fetching shift", error: err.message});
    })
})

//http://localhost:8080/shift/update/id
//Update one record
router.route("/update/:id").put(async (req, res) => {
    ...
    .then(() => {
        ...
        console.log(err);
        res.status(500).send({status: "Error with updating shift", error: err.message});
    }) 
})

//http://localhost:8080/shift/delete/id
//Delete one record
router.route("/delete/:id").delete(async (req, res) => {
    ...
    .then(() => {
        ...
        console.log(err.message);
        res.status(500).send({status: "Error with deleting shift", error: err.message});
    })
})
