...
//http://localhost:8070/department/create
...
//http://localhost:8070/department/read
...
//http://localhost:8070/department/read/id
//Read data from the database
...
router.route("/read/:id").get(async (req, res) => {
  ...
  const department = await Department.findById(depId)
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .send({ status: "Error with fetching department", error: err.message });
    });
});

//http://localhost:8070/department/update/id
//Update one record
router.route("/update/:id").put(async (req, res) => {
  ...
  const update = await Department.findByIdAndUpdate(depId, updateDepartment)
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .send({ status: "Error with updating department", error: err.message });
    });
});

//http://localhost:8070/department/delete/id
//Delete one record
router.route("/delete/:id").delete(async (req, res) => {
  ...
  await Department.findByIdAndDelete(depId)
    ...
    .catch((err) => {
      ...
      res
        .status(500)
        .send({ status: "Error with deleting department", error: err.message });
    });
});
