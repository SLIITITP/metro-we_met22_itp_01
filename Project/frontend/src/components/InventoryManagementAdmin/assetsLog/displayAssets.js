...
import GetAssetDetails from "../../InventoryManagement/AssetsLog/Content/getAllAssets";
import { Button } from "react-bootstrap";
import {jsPDF} from "jspdf";
export default function DisplayAssets() {
  
    const createPDF = async () => {
        const date = new Date().toISOString().split('T')[0];
        const pdf = new jsPDF("landscape", "px", "a1", false);
        const data = await document.querySelector("#assets");
        pdf.html(data).then(() => {
            pdf.save("Assets Report" + date + ".pdf");
        });
    };
